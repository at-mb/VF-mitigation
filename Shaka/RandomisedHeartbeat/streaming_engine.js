...
shaka.media.StreamingEngine = class {
  ...
  createMediaState_(stream) {
    ...
    const mediaState = {
      ...
      // Mitigation: true while the async burst loop is executing for this
      // stream; prevents ghost timers from re-entering update_().
  ...
  update_(mediaState) {
    ...
    // =========================================================================
    // Bloc 4: Randomised decision — how many segments to fetch this cycle.
    //
    // Buffer zones:
    //   < UNDERFLOW  →  refill mode:      50% fetch-1,  50% fetch-2
    //   UNDERFLOW–OVERFLOW → obfuscation: 25% fetch-0,  50% fetch-1, 25% fetch-2
    //   OVERFLOW–HARD_CAP → drain mode:   50% fetch-0,  50% fetch-1
    //   ≥ HARD_CAP   →  stop mode:        always fetch-0
    //
    // The Handshake ensures audio and video share the same decision each cycle:
    // the first stream to reach this point in a cycle is the "Leader" and posts
    // its choice to mitigation_sharedDecision_; the second stream is the
    // "Follower" and copies that choice.
    // =========================================================================
    ...
    if (this.mitigation_sharedDecision_ >= 0 &&
      ...
      // Follower: copy the partner's decision and reset the mailbox.
      mitigation_segmentsToFetch = this.mitigation_sharedDecision_;
      this.mitigation_sharedDecision_ = -1;
      this.mitigation_decisionMaker_ = null;
    } else {
      // Leader: make a randomised decision and post it for the partner.
      const rand = Math.random();
      if (bufferedAhead >= HARD_CAP_THRESHOLD) {
        mitigation_segmentsToFetch = 0;
      } else if (bufferedAhead >= OVERFLOW_THRESHOLD) {
        mitigation_segmentsToFetch = rand < 0.5 ? 0 : 1;
      } else if (bufferedAhead >= UNDERFLOW_THRESHOLD) {
        // 25 % chance of 0, 50 % chance of 1, 25 % chance of 2
        mitigation_segmentsToFetch = rand < 0.25 ? 0 : (rand < 0.75 ? 1 : 2);
      } else {
        mitigation_segmentsToFetch = rand < 0.5 ? 1 : 2;
      }
    ...
    // calls are replaced by the burst loop below.  The Handshake above (Bloc 4)
    // replaces the dependency-based synchronisation.
    //
    // Bloc 6: Robust async burst loop — the "Heart".
    //
    // Key properties:
    //  • Fetches mitigation_segmentsToFetch segments (0–2) sequentially.
    //  • Sets mitigation_inBurstLoop_ = true (Bloc 2 guard) before any await.
    //  • After each fetchAndAppend_ the immediate re-schedule is cancelled so
    //    the burst loop, not the original timer, controls the next wake-up.
    //  • Fails safely: stops the burst early if the fetch was aborted, the
    //    buffer is about to be cleared, or the engine has been destroyed.
    //  • Fixed heartbeat: the finally block schedules the next update after
    //    exactly one max-segment-duration (≈ segment length), regardless of how
    //    many segments were fetched.  This destroys the timing side-channel.
    //  • Fast-start: if the buffer is critically low after the burst, the
    //    heartbeat delay is 0 so playback can start immediately.
    // =========================================================================
    const mitigation_segmentsToFetchNow = mitigation_segmentsToFetch;
    ...
    const runBurstLoop = async () => {
      // Set the guard synchronously (before the first await) so that any timer
      // firing immediately after this call sees inBurstLoop_ = true.
      mediaState.mitigation_inBurstLoop_ = true;
      try {
        let currentRef = mitigation_initialReference;
        for (let i = 0; i < mitigation_segmentsToFetchNow; i++) {
          // Abort early on clear-buffer requests or player destruction.
          ...
          // Pass adaptation=true only for the first segment of the burst;
          // subsequent segments are continuations, not stream-switch points.
          await this.fetchAndAppend_(
              mediaState, pt, currentRef,
              i === 0 ? mitigation_initialAdaptation : false);

          // fetchAndAppend_ schedules an immediate re-update on success or
          // abort; cancel it so the burst loop controls the next wake-up.
          this.cancelUpdate_(mediaState);

          // Detect a failed/aborted fetch: lastSegmentReference is only
          // updated on success, so equality with the pre-fetch value means
          // the segment was not appended.
          if (mediaState.lastSegmentReference === refBeforeFetch) {
            break;
          }

          // Prepare the reference for the next iteration (if there is one).
          if (i + 1 < mitigation_segmentsToFetchNow) {
        ...
        // Only schedule the heartbeat if the existing timer machinery has not
        // already done so (e.g., a seek or buffer-clear will have set
        // updateTimer or clearingBuffer, and that mechanism handles its own
        // re-scheduling).
        if (this.playerInterface_ &&
            mediaState.updateTimer == null &&
          ...
          const currentBuffered =
              this.playerInterface_.mediaSourceEngine.bufferedAheadOf(
                  mediaState.type, pt);
          // Fast-start: skip the heartbeat delay while still in underflow so
          // the player fills the buffer quickly after startup or a seek.
          const heartbeat =
