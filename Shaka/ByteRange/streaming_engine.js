...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    // Mitigation state — video fingerprinting side-channel defence.
    // Shared mailbox: the first stream to run update_() each cycle posts its
    // download decision here so the partner can copy it (Handshake, Bloc 4).
    // -1 means "no decision posted yet this cycle".
    /** @private {number} */
    this.mitigation_sharedDecision_ = -1;

    // Which stream type acted as the leader for the current mailbox value.
    // Prevents a stream from accidentally reading its own stale decision.
    /** @private {?string} */
    this.mitigation_decisionMaker_ = null;
  }
  ...
  createMediaState_(stream) {
    ...
    const mediaState = {
      ...
      // Mitigation state for continuous byte-range download strategy.
      mitigation_inBurstLoop_: false,    // burst guard
      mitigation_carryRef_: null,        // segment partially downloaded this cycle
      mitigation_carryByteOffset_: 0,   // bytes already fetched from carryRef
      mitigation_carryData_: null,       // accumulated partial bytes (Uint8Array)
      mitigation_observedSegSize_: 0,   // actual bytes of last fully-fetched segment
    };
    if (stream.dependencyStream) {
      mediaState.dependencyMediaState =
          this.createMediaState_(stream.dependencyStream);
  ...
  update_(mediaState) {
    ...
    // Bloc 2: Burst Guard — prevents ghost timers (gap-fix retries, etc.) from
    // re-entering update_() while our async burst loop is running, which would
    // cause assertion failures in fetchAndAppend_.
    if (mediaState.mitigation_inBurstLoop_) {
      return null;
    }
    ...
    // Bloc 3: bypass the original early-return when buffering goal is met.
    // The fixed-heartbeat burst loop enforces its own timing, so we must always
    // fall through to the mitigation decision logic regardless of buffer level.
    if (bufferedAhead >= scaledBufferingGoal) {
      shaka.log.v2(logPrefix, 'buffering goal met (mitigation: continuing)');
    }
    ...
    // =========================================================================
    // Bloc 4: Randomised download decision for this cycle.
    //
    // Two regimes based on buffer level:
    //
    //   UNDERFLOW (< 8 s): integer {1, 2} complete segments — identical to the
    //     original discrete strategy.  No partial downloads.  This guarantees
    //     the buffer fills fast and playback starts reliably.
    //
    //   BUFFERED (≥ 8 s): continuous byte-range strategy.  We draw a fraction
    //     f from a buffer-level-dependent Uniform distribution and convert it
    //     to a byte budget.  Carry-over state lets the previous cycle's
    //     partially-downloaded segment spill into this cycle, so the network
    //     never sees whole-segment-aligned burst sizes.  The fingerprint space
    //     becomes infinite (no enumerable Fibonacci-bounded set).
    //
    //     NORMAL  (8–24 s):  f ~ Uniform[0, 2],  E = 1.0 seg/cycle
    //     OVERFLOW (24–28 s): f ~ Uniform[0, 1],  E = 0.5 seg/cycle
    //     HARD_CAP (≥ 28 s):  f = 0  (stop)
    //
    // Handshake: the first stream to reach this point ("Leader") posts its
    // decision to the shared mailbox; the second stream ("Follower") copies it
    // so audio and video always download the same fraction each cycle.
    // =========================================================================
    const UNDERFLOW_THRESHOLD = 8;
    const OVERFLOW_THRESHOLD = 24;
    const HARD_CAP_THRESHOLD = 28;

    // Discard stale carry-over when a seek moved us past the carried segment.
    if (mediaState.mitigation_carryRef_ &&
        reference !== mediaState.mitigation_carryRef_) {
      mediaState.mitigation_carryRef_ = null;
      mediaState.mitigation_carryByteOffset_ = 0;
      mediaState.mitigation_carryData_ = null;
    }

    // ---- Underflow: integer segments, no carry-over ----
    if (bufferedAhead < UNDERFLOW_THRESHOLD) {
      let mitigation_segmentsToFetch;
      if (this.mitigation_sharedDecision_ >= 0 &&
          this.mitigation_decisionMaker_ !== mediaState.type) {
        mitigation_segmentsToFetch = this.mitigation_sharedDecision_;
        this.mitigation_sharedDecision_ = -1;
        this.mitigation_decisionMaker_ = null;
      } else {
        mitigation_segmentsToFetch = Math.random() < 0.5 ? 1 : 2;
        this.mitigation_sharedDecision_ = mitigation_segmentsToFetch;
        this.mitigation_decisionMaker_ = mediaState.type;
      }

      const mitigation_n = mitigation_segmentsToFetch;
      const mitigation_initialReference = reference;
      const mitigation_initialAdaptation = adaptation;
      const mitigation_maxSegDuration =
          this.manifest_.presentationTimeline.getMaxSegmentDuration();

      const runBurstLoop = async () => {
        mediaState.mitigation_inBurstLoop_ = true;
        try {
          let currentRef = mitigation_initialReference;
          for (let i = 0; i < mitigation_n; i++) {
            if (!currentRef || mediaState.waitingToClearBuffer ||
                !this.playerInterface_) {
              break;
            }
            const refBeforeFetch = mediaState.lastSegmentReference;
            const pt = this.playerInterface_.getPresentationTime();
            await this.fetchAndAppend_(
                mediaState, pt, currentRef,
                i === 0 ? mitigation_initialAdaptation : false);
            this.cancelUpdate_(mediaState);
            if (mediaState.lastSegmentReference === refBeforeFetch) {
              break;
            }
            if (i + 1 < mitigation_n) {
              const bufEnd = this.playerInterface_.mediaSourceEngine.bufferEnd(
                  mediaState.type);
              const nextPt = this.playerInterface_.getPresentationTime();
              currentRef = this.getSegmentReferenceNeeded_(
                  mediaState, nextPt, bufEnd);
            }
          }
        } finally {
          mediaState.mitigation_inBurstLoop_ = false;
          if (this.playerInterface_ &&
              mediaState.updateTimer == null &&
            ...
            const currentBuffered =
                this.playerInterface_.mediaSourceEngine.bufferedAheadOf(
                    mediaState.type, pt);
            const heartbeat =
                currentBuffered < UNDERFLOW_THRESHOLD ? 0 : mitigation_maxSegDuration;
            this.scheduleUpdate_(mediaState, heartbeat);
          }
        }
      };
      runBurstLoop().catch(() => {});
    ...
    // ---- Buffered: continuous byte-range strategy ----

    let mitigation_f;
    if (this.mitigation_sharedDecision_ >= 0 &&
        this.mitigation_decisionMaker_ !== mediaState.type) {
      // Follower: copy partner's decision and reset the mailbox.
      mitigation_f = this.mitigation_sharedDecision_;
      this.mitigation_sharedDecision_ = -1;
      this.mitigation_decisionMaker_ = null;
    } else {
      // Leader: draw f from the buffer-level-appropriate distribution.
      if (bufferedAhead >= HARD_CAP_THRESHOLD) {
        mitigation_f = 0;
      } else if (bufferedAhead >= OVERFLOW_THRESHOLD) {
        mitigation_f = Math.random();        // Uniform[0,1], E = 0.5
      } else {
        mitigation_f = Math.random() * 2;   // Uniform[0,2], E = 1.0
      }
      this.mitigation_sharedDecision_ = mitigation_f;
      this.mitigation_decisionMaker_ = mediaState.type;
    }

    // =========================================================================
    // Blocs 5 & 7: the original fetchAndAppend_ / fetchAndAppendDependency_
    // calls are replaced by the byte-budget burst loop below.
    //
    // Bloc 6: Async byte-budget burst loop — the "Heart".
    //
    // Key properties:
    //  • Computes byteBudget = f × estimatedSegmentSize.
    //  • Part A: if carry-over exists, complete or extend it.
    //    Completing: fetch the tail bytes, assemble the full segment, inject
    //    the data into the reference via setSegmentData so that fetchAndAppend_
    //    appends it without a new network request.
    //    Extending: download more bytes and return; heartbeat fires below.
    //  • Part B: with remaining budget, fetch whole segments (fetchAndAppend_)
    //    or start a new partial carry-over (mitigation_fetchRange_).
    //  • Fixed heartbeat: the finally block schedules the next update after
    //    exactly maxSegmentDuration regardless of how many bytes were fetched.
    //  • Fast-start guard: heartbeat = 0 if buffer drops below UNDERFLOW_THRESHOLD.
    //  • Seek safety: carry-over is discarded in the finally block on clear.
    // =========================================================================
    const mitigation_initialReference = reference;
    const mitigation_initialAdaptation = adaptation;
    const mitigation_maxSegDuration =
    ...
    // Estimate segment size in bytes.  Priority:
    //   1. Manifest-explicit byte range (most accurate).
    //   2. Observed size from the last fully-fetched segment of this stream.
    //   3. bandwidth × segment duration (least accurate; used on cold start).
    //
    //  For bandwidth: mirror the existing Shaka pattern (line ~896): prefer
    //  per-stream bandwidth, fall back to variant bandwidth (covers HLS and
    //  DASH manifests that omit per-stream values), then fall back to the ABR
    //  engine's live estimate.
    const mitigation_getSegSize = (ref) => {
      if (ref.endByte !== null && ref.endByte !== undefined) {
        return ref.endByte - ref.startByte + 1;
      }
      if (mediaState.mitigation_observedSegSize_ > 0) {
        return mediaState.mitigation_observedSegSize_;
      }
      const bw =
          mediaState.stream.bandwidth ||
          (this.currentVariant_ && this.currentVariant_.bandwidth) ||
          this.playerInterface_.getBandwidthEstimate() ||
          0;
      const segDuration = ref.endTime - ref.startTime;
      const bwEstimate = Math.max(1024, Math.ceil(bw / 8 * segDuration));
      console.log('[mitigation] segSize cold-start estimate', bwEstimate, 'B',
          'for', mediaState.type,
          '| streamBw=' + mediaState.stream.bandwidth +
          ' variantBw=' + (this.currentVariant_ && this.currentVariant_.bandwidth) +
          ' abrBw=' + Math.round(this.playerInterface_.getBandwidthEstimate()) +
          ' segDur=' + segDuration.toFixed(2) + 's');
      return bwEstimate;
    };

    const mitigation_refForSize =
        mediaState.mitigation_carryRef_ || mitigation_initialReference;
    const mitigation_segSize = mitigation_getSegSize(mitigation_refForSize);
    const mitigation_byteBudget = mitigation_f * mitigation_segSize;
    console.log('[mitigation] cycle', mediaState.type,
        'buffered=' + bufferedAhead.toFixed(1) + 's',
        'f=' + mitigation_f.toFixed(3),
        'segSize=' + mitigation_segSize + 'B',
        'budget=' + Math.round(mitigation_byteBudget) + 'B',
        mediaState.mitigation_carryRef_ ?
            'carry@' + mediaState.mitigation_carryByteOffset_ + 'B' : 'no-carry');

    const runBurstLoop = async () => {
      mediaState.mitigation_inBurstLoop_ = true;

      try {
        if (mitigation_byteBudget <= 0) {
          return;  // Nothing to fetch this cycle; heartbeat fires below.
        }

        let remainingBytes = mitigation_byteBudget;
        let fetchedAdaptation = false;

        // -----------------------------------------------------------------
        // Part A: Continue or complete a carry-over partial segment.
        // -----------------------------------------------------------------
        let currentRef;

        if (mediaState.mitigation_carryRef_) {
          const carryRef = mediaState.mitigation_carryRef_;
          const carryOffset = mediaState.mitigation_carryByteOffset_;
          const carryData = mediaState.mitigation_carryData_ ||
              new Uint8Array(0);

          if (mediaState.waitingToClearBuffer || !this.playerInterface_) {
            return;
          }

          const carrySegSize = mitigation_getSegSize(carryRef);
          const remainingInSeg = carrySegSize - carryOffset;

          if (remainingBytes >= remainingInSeg) {
            // Complete the carry-over: fetch tail, assemble, inject, append.
            console.log('[mitigation] Part A complete carry', mediaState.type,
                'carryOffset=' + carryOffset + 'B',
                'tailBytes=' + remainingInSeg + 'B');
            const tailData = await this.mitigation_fetchRange_(
                mediaState, carryRef, carryOffset, null);
            this.cancelUpdate_(mediaState);
            ...
            const fullData = shaka.util.Uint8ArrayUtils.concat(
                carryData, tailData);
            console.log('[mitigation] assembled segment', mediaState.type,
                'carry=' + carryData.byteLength + 'B',
                'tail=' + tailData.byteLength + 'B',
                'total=' + fullData.byteLength + 'B');
            // Inject assembled data so fetchAndAppend_ skips the network.
            carryRef.setSegmentData(fullData, /* singleUse= */ true);
            ...
            const pt = this.playerInterface_.getPresentationTime();
            await this.fetchAndAppend_(
                mediaState, pt, carryRef,
                mitigation_initialAdaptation && !fetchedAdaptation);
            this.cancelUpdate_(mediaState);
            fetchedAdaptation = true;

            if (mediaState.lastSegmentReference === refBeforeFetch) {
              console.warn('[mitigation] carry append failed', mediaState.type,
                  'assembled=' + fullData.byteLength + 'B — segment not advanced');
              return;  // Fetch failed or was aborted.
            }

            // Record the observed segment size for future budget estimates.
            mediaState.mitigation_observedSegSize_ = fullData.byteLength;
            console.log('[mitigation] carry appended OK', mediaState.type,
                'observedSegSize updated to', fullData.byteLength + 'B');

            remainingBytes -= remainingInSeg;

            // Compute the next reference for Part B.
            if (remainingBytes > 0 && !mediaState.waitingToClearBuffer &&
                this.playerInterface_) {
              const bufEnd = this.playerInterface_.mediaSourceEngine.bufferEnd(
                  mediaState.type);
              const nextPt = this.playerInterface_.getPresentationTime();
            ...
            // Budget exhausted inside the carry-over: extend the partial.
            const newOffset = carryOffset + Math.floor(remainingBytes);
            console.log('[mitigation] Part A extend carry', mediaState.type,
                carryOffset + 'B →', newOffset + 'B');
            const moreData = await this.mitigation_fetchRange_(
                mediaState, carryRef, carryOffset, newOffset);
            this.cancelUpdate_(mediaState);

            if (!mediaState.waitingToClearBuffer && this.playerInterface_) {
              const expectedMore = newOffset - carryOffset;
              if (moreData.byteLength < expectedMore) {
                // The extension request reached the end of the actual segment
                // (newOffset exceeded the file boundary).  We now have all
                // remaining bytes — assemble the full segment and inject it.
                const fullData = shaka.util.Uint8ArrayUtils.concat(
                    carryData, moreData);
                mediaState.mitigation_observedSegSize_ = fullData.byteLength;
                console.log('[mitigation] Part A extend → got full segment',
                    mediaState.type,
                    'got=' + moreData.byteLength + 'B < expected=' + expectedMore + 'B',
                    'assembled=' + fullData.byteLength + 'B — injecting directly');
                mediaState.mitigation_carryRef_ = null;
                mediaState.mitigation_carryByteOffset_ = 0;
                mediaState.mitigation_carryData_ = null;
                carryRef.setSegmentData(fullData, /* singleUse= */ true);
                ...
                this.cancelUpdate_(mediaState);
                if (mediaState.lastSegmentReference === refBeforeFetch) {
                  console.warn('[mitigation] Part A extend direct inject failed',
                      mediaState.type);
                } else {
                  fetchedAdaptation = true;
                  console.log('[mitigation] Part A extend direct inject OK',
                      mediaState.type);
                }
              } else {
                mediaState.mitigation_carryData_ =
                    shaka.util.Uint8ArrayUtils.concat(carryData, moreData);
                mediaState.mitigation_carryByteOffset_ = newOffset;
                console.log('[mitigation] carry extended', mediaState.type,
                    'accumulated=' + mediaState.mitigation_carryData_.byteLength + 'B',
                    'offset=' + newOffset + 'B');
              }
            }
          ...
          currentRef = mitigation_initialReference;
        }

        // -----------------------------------------------------------------
        // Part B: Download new whole or partial segments with remaining budget.
        // -----------------------------------------------------------------
        while (remainingBytes > 0 && currentRef &&
            !mediaState.waitingToClearBuffer && this.playerInterface_) {
          const refSegSize = mitigation_getSegSize(currentRef);

          if (remainingBytes >= refSegSize) {
            // Fetch the full segment and measure its actual byte count so that
            // mitigation_observedSegSize_ converges to a real value rather than
            // the bandwidth estimate.
            //
            // We call mitigation_fetchRange_(0, null): start=0 and end=null
            // makes createSegmentRequest emit NO Range header (same as a
            // normal full-segment download).  The return value gives us the
            // exact byte count.  We then inject the bytes via setSegmentData
            // so that the subsequent fetchAndAppend_ call skips the network.
            //
            // Exception: if the segment already has injected data (e.g. from a
            // previous carry-over assembly) we skip the extra download.
            console.log('[mitigation] Part B full fetch', mediaState.type,
                'refSegSize=' + refSegSize + 'B',
                'remainingBudget=' + Math.round(remainingBytes) + 'B');

            const alreadyHasData = !!currentRef.getSegmentData(
                /* allowDeleteOnSingleUse= */ false);
            if (!alreadyHasData) {
              const fullBytes = await this.mitigation_fetchRange_(
                  mediaState, currentRef, 0, null);
              this.cancelUpdate_(mediaState);
              ...
              mediaState.mitigation_observedSegSize_ = fullBytes.byteLength;
              console.log('[mitigation] Part B measured actual segSize',
                  fullBytes.byteLength + 'B for', mediaState.type);
              currentRef.setSegmentData(fullBytes, /* singleUse= */ true);
            }
            ...
            if (mediaState.lastSegmentReference === refBeforeFetch) {
              console.warn('[mitigation] Part B full fetch failed', mediaState.type);
              break;  // Fetch failed or was aborted.
            }

            console.log('[mitigation] Part B full OK', mediaState.type,
                'observedSegSize=' + mediaState.mitigation_observedSegSize_ + 'B');

            remainingBytes -= refSegSize;
            ...
            // Partial fetch: start a new carry-over for the next cycle.
            const partialEnd = Math.floor(remainingBytes);
            if (partialEnd <= 0) {
              break;
            }
            console.log('[mitigation] Part B partial fetch', mediaState.type,
                '0 →', partialEnd + 'B', 'of est.', refSegSize + 'B');
            const partialData = await this.mitigation_fetchRange_(
                mediaState, currentRef, 0, partialEnd);
            this.cancelUpdate_(mediaState);

            if (!mediaState.waitingToClearBuffer && this.playerInterface_) {
              if (partialData.byteLength < partialEnd) {
                // The partial request exceeded the actual segment boundary —
                // the server returned the whole segment (fewer bytes than
                // requested).  Skip carry-over: inject directly and append.
                mediaState.mitigation_observedSegSize_ = partialData.byteLength;
                console.log('[mitigation] Part B partial → got full segment',
                    mediaState.type,
                    'got=' + partialData.byteLength + 'B < requested ' + partialEnd + 'B',
                    '— injecting directly, observedSegSize corrected');
                currentRef.setSegmentData(partialData, /* singleUse= */ true);
                const refBeforeFetch = mediaState.lastSegmentReference;
                const pt = this.playerInterface_.getPresentationTime();
                await this.fetchAndAppend_(
                    mediaState, pt, currentRef,
                ...
                this.cancelUpdate_(mediaState);
                if (mediaState.lastSegmentReference === refBeforeFetch) {
                  console.warn('[mitigation] Part B direct inject failed',
                      mediaState.type);
                } else {
                  fetchedAdaptation = true;
                  console.log('[mitigation] Part B direct inject OK',
                      mediaState.type,
                      'observedSegSize=' + partialData.byteLength + 'B');
                }
              } else {
                // Normal partial: stash for the next cycle's carry-over.
                mediaState.mitigation_carryRef_ = currentRef;
                mediaState.mitigation_carryByteOffset_ = partialEnd;
                mediaState.mitigation_carryData_ = partialData;
                console.log('[mitigation] carry started', mediaState.type,
                    'got=' + partialData.byteLength + 'B',
                    'expected=' + partialEnd + 'B OK');
              }
            }
        ...
        // On seek or buffer-clear, discard carry-over so the next cycle
        // starts fresh from wherever the playhead landed.
  ...
  /**
   * Fetches a byte range from a segment reference for the mitigation burst
   * loop.  Returns the raw (possibly partial) response bytes as a Uint8Array.
   *
   * `startOffset` and `endOffset` are expressed in bytes relative to the start
   * of the segment (i.e. relative to reference.startByte), not relative to the
   * start of the underlying file.  Pass `null` for `endOffset` to fetch to the
   * end of the segment.
   *
   * @param {!shaka.media.StreamingEngine.MediaState_} mediaState
   * @param {!shaka.media.SegmentReference} reference
   * @param {number} startOffset  Byte offset within segment (0-based).
   * @param {?number} endOffset   Exclusive end offset within segment, or null.
   * @return {!Promise<!Uint8Array>}
   * @private
   */
  async mitigation_fetchRange_(mediaState, reference, startOffset, endOffset) {
    const segBase = reference.startByte || 0;
    const actualStart = segBase + startOffset;
    // endOffset is exclusive; the Range header end byte is inclusive.
    const actualEnd = (endOffset !== null && endOffset !== undefined)
        ? segBase + endOffset - 1
        : reference.endByte;  // null means "to end of segment" (open-ended range)

    const rangeDesc = actualEnd !== null
        ? `bytes=${actualStart}-${actualEnd}`
        : `bytes=${actualStart}-`;
    console.log('[mitigation] fetchRange =', mediaState.type, rangeDesc,
        'segBase=' + segBase,
        endOffset !== null
    ...
    request.contentType = mediaState.stream.type;
    ...
    try {
      const response = await op.promise;
      let data = shaka.util.BufferUtils.toUint8(response.data);

      console.log('[mitigation] fetchRange response', mediaState.type,
          'status=' + response.status,
          'got=' + data.byteLength + 'B',
          'requested ~' + (actualEnd !== null
              ? actualEnd - actualStart + 1
              : '?') + 'B');

      // If the server returned 200 (full content) instead of 206 (partial),
      // it ignored our Range header.  For separate-file segments (segBase === 0)
      // the response starts at byte 0 of the segment, so we can slice manually.
      // For byte-range container files (segBase > 0) the server should always
      // return 206 since that is a core DASH requirement; log a warning and
      // return as-is (the data will likely be wrong, but there is nothing we
      // can do without knowing the full file size).
      if (response.status !== 206 && segBase === 0) {
        const sliceEnd = endOffset !== null ? endOffset : data.length;
        console.warn('[mitigation] server returned', response.status,
            '(no Range support) — slicing manually',
            `[${startOffset}..${sliceEnd}) of ${data.byteLength}B response`);
        data = data.slice(startOffset, sliceEnd);
        console.log('[mitigation] sliced to', data.byteLength + 'B');
      } else if (response.status !== 206 && segBase > 0) {
        console.warn('[mitigation] server returned', response.status,
            'for byte-range container segment (segBase=' + segBase + ') —',
            'expected 206; assembled segment data may be corrupt');
      }
 ...
 *   mitigation_inBurstLoop_: boolean,
 *   mitigation_carryRef_: ?shaka.media.SegmentReference,
 *   mitigation_carryByteOffset_: number,
 *   mitigation_carryData_: ?Uint8Array,
 *   mitigation_observedSegSize_: number,
 * }}
 *
 * @description
 ...
 * @property {boolean} mitigation_inBurstLoop_
 *   True while the mitigation async burst loop is executing for this stream.
 *   Prevents ghost timers from re-entering update_() mid-burst.
 * @property {?shaka.media.SegmentReference} mitigation_carryRef_
 *   The segment currently being partially downloaded across burst cycles.
 *   Null when no partial download is in progress.
 * @property {number} mitigation_carryByteOffset_
 *   Number of bytes already fetched from mitigation_carryRef_.
 * @property {?Uint8Array} mitigation_carryData_
 *   Accumulated bytes fetched so far for mitigation_carryRef_.
 */
shaka.media.StreamingEngine.MediaState_;
 ...
 * @private
 */
// Raised from 1 to 2 to allow the burst loop to buffer up to 2 segments ahead.
shaka.media.StreamingEngine.MAX_RUN_AHEAD_SEGMENTS_ = 2;
