...
shaka.media.StreamingEngine = class {
  ...
  constructor(manifest, playerInterface) {
    ...
    /** @private {number} */
    this.mitigation_segmentsToFetch_ = 1; // declaring a shared variable of how much segments to fetch (to have #video segments == #audio segments)
    /** @private {boolean} */
    this.mitigation_audioBusy_ = false; // *TO BE VERIFIED* synchronizing audio segments with video segments
    /** @private {boolean} */
    this.mitigation_videoBusy_ = false; // *TO BE VERIFIED* synchronizing video segments with audio segments
    /** @private {number} */
    this.mitigation_sharedDecision_ = null; // The number to download
    /** @private {number} */
    this.mitigation_decisionMaker_ = null;    // Who decided it ('video' or 'audio')
    /// === END OF SOLUTION BLOC 1 ===
  ...
  createMediaState_(stream) {
    ...
    const mediaState = {
      ...
      /// === SOLUTION BLOC 1 ===
      // Declaring a variable to detect when we enter in a burst
      mitigation_inBurstLoop_: false,
      /// === END OF SOLUTION BLOC 1 ===
    };
    if (stream.dependencyStream) {
      mediaState.dependencyMediaState =
  ...
  update_(mediaState) {
    ...
    // ... [Standard Shaka Checks: Text, AudioMuxed, Closed, StreamingAllowed] ...
    ...
    if (mediaState.stream.isAudioMuxedInVideo && mediaState.type == ContentType.AUDIO) return null;
    if (mediaState.type != ContentType.TEXT && this.playerInterface_.mediaSourceEngine.closed()) {
      this.resetMediaSource(true);
      return null;
    }

    const updateIntervalSeconds = this.config_.updateIntervalSeconds / Math.max(1, Math.abs(this.playerInterface_.getPlaybackRate()));
    ...
    // Disable Native Audio Prefetch
    /*
    if (mediaState.type === ContentType.AUDIO) {
      for (const stream of this.audioPrefetchMap_.keys()) {
        const prefetch = this.audioPrefetchMap_.get(stream);
        prefetch.evict(presentationTime, true);
    ...
    */
    ...
    if (timeUntilEnd < 1e-6 && !!bufferEnd) {
      mediaState.endOfStream = true;
      if (mediaState.type == ContentType.VIDEO) {
        const textState = this.mediaStates_.get(ContentType.TEXT);
        if (textState && shaka.media.StreamingEngine.isEmbeddedText_(textState)) textState.endOfStream = true;
      }
      return null;
    }
    mediaState.endOfStream = false;

    /// === SOLUTION BLOC 4: DECISION LOGIC ===
    const UNDERFLOW_THRESHOLD = 8; 
    const OVERFLOW_THRESHOLD = 24; 
    const HARD_CAP_THRESHOLD = OVERFLOW_THRESHOLD + 4; 

    // Too Far Ahead Check
    let minTimeNeededByOthers = Infinity;
    const allMediaStates = Array.from(this.mediaStates_.values());
    for (const ms of allMediaStates) {
        if (ms !== mediaState && !shaka.media.StreamingEngine.isEmbeddedText_(ms)) {
             const otherTime = this.getTimeNeeded_(ms, presentationTime);
             minTimeNeededByOthers = Math.min(minTimeNeededByOthers, otherTime);
        }
    }

    const maxSegmentDuration = this.manifest_.presentationTimeline.getMaxSegmentDuration();
    if (timeNeeded > minTimeNeededByOthers + (maxSegmentDuration * 1.5)) {
         shaka.log.v1(logPrefix, 'Mitigation: Stream too far ahead. Waiting.');
         this.mitigation_segmentsToFetch_ = 0;
         this.scheduleUpdate_(mediaState, updateIntervalSeconds);
         return null; 
    }

    const otherType = (mediaState.type == ContentType.VIDEO) ? ContentType.AUDIO : ContentType.VIDEO;
    let hasPartner = this.mediaStates_.has(otherType);
    if (mediaState.type == ContentType.VIDEO && mediaState.stream.isAudioMuxedInVideo) {
        hasPartner = false;
    }

    this.mitigation_shouldFetchDecoy_ = false; 

    // Warm-Up Logic
    if (typeof mediaState.mitigation_warmupCount_ === 'undefined') {
        mediaState.mitigation_warmupCount_ = 0;
    }
    const isWarmingUp = (mediaState.mitigation_warmupCount_ < 2);

    if (mediaState.needInitSegment || isWarmingUp) {
        this.mitigation_segmentsToFetch_ = 1;
        this.mitigation_shouldFetchDecoy_ = false;
    } 
    else if (hasPartner && this.mitigation_sharedDecision_ !== null) {
        if (this.mitigation_decisionMaker_ === mediaState.type) {
             return updateIntervalSeconds / 4; 
        } else {
            this.mitigation_segmentsToFetch_ = this.mitigation_sharedDecision_;
            this.mitigation_shouldFetchDecoy_ = this.mitigation_sharedDecoyDecision_; 
            this.mitigation_sharedDecision_ = null;
            this.mitigation_sharedDecoyDecision_ = null;
        ...
        if (bufferedAhead < UNDERFLOW_THRESHOLD) { 
          calculatedSegments = (rand < 0.5) ? 1 : 2; 
        } 
        else if (bufferedAhead > HARD_CAP_THRESHOLD) {
          calculatedSegments = 0; 
        } 
        else if (bufferedAhead > OVERFLOW_THRESHOLD) {
          calculatedSegments = (rand < 0.5) ? 0 : 1; 
        } 
        else {
          isNormalZone = true;
          if (rand < 0.25) calculatedSegments = 0;
          else if (rand < 0.50) calculatedSegments = 2;
          else calculatedSegments = 1;
        }

        this.mitigation_segmentsToFetch_ = calculatedSegments;
        
        if (isNormalZone && Math.random() < 0.2) {
             this.mitigation_shouldFetchDecoy_ = true;
        }
        
        if (hasPartner) {
            this.mitigation_sharedDecision_ = calculatedSegments;
            this.mitigation_sharedDecoyDecision_ = this.mitigation_shouldFetchDecoy_;
            this.mitigation_decisionMaker_ = mediaState.type;
        }
    }
    /// === END OF SOLUTION BLOC 4 ===
    ...
    if (!reference) return updateIntervalSeconds;

    const lastInitSegRef = mediaState.lastInitSegmentReference;
    const initSegRef = reference.initSegmentReference;
    if (lastInitSegRef && initSegRef && shaka.media.InitSegmentReference.equal(initSegRef, lastInitSegRef)) {
    ...
    for (const otherState of mediaStates) {
      if (shaka.media.StreamingEngine.isEmbeddedText_(otherState)) continue;
      if (otherState.segmentIterator && !otherState.segmentIterator.current()) continue;
      minTimeNeeded = Math.min(minTimeNeeded, this.getTimeNeeded_(otherState, presentationTime));
    }
    const maxRunAhead = maxSegmentDuration * shaka.media.StreamingEngine.MAX_RUN_AHEAD_SEGMENTS_;
    if (timeNeeded >= minTimeNeeded + maxRunAhead) return updateIntervalSeconds;
    ...
    if (mediaState.segmentPrefetch && mediaState.segmentIterator && !this.audioPrefetchMap_.has(mediaState.stream)) {
      ...
      mediaState.segmentPrefetch.prefetchSegmentsByTime(reference.startTime).catch(() => {});
    }
    */

    if (this.shouldUseCrossBoundaryLogic_() && this.discardReferenceByBoundary_(mediaState, reference)) return null;


    /// === SOLUTION BLOC 6: HYBRID EXECUTION ===
    (async () => {
      // 1. Lock Flags
      if (mediaState.type == ContentType.VIDEO) this.mitigation_videoBusy_ = true;
      else if (mediaState.type == ContentType.AUDIO) this.mitigation_audioBusy_ = true;
      mediaState.mitigation_inBurstLoop_ = true;

      if (mediaState.waitingToClearBuffer) {
          this.cleanupAndReschedule_(mediaState, maxSegmentDuration);
          return;
      }
      ...
      const currentStream = mediaState.stream;

      // === BRANCH A: SIMPLE PATH ===
      if (this.mitigation_segmentsToFetch_ === 1 && !this.mitigation_shouldFetchDecoy_) {
          try {
              // Always use the 'reference' calculated by getSegmentReferenceNeeded_
              // This is the source of truth for time alignment.
              const refToFetch = reference; 
              
              if (refToFetch) {
                  mediaState.performingUpdate = false;
                  await this.fetchAndAppend_(mediaState, presentationTime, refToFetch, adaptation);
                  mediaState.performingUpdate = true;

                  // Sync iterator state for next time
                  // If seek/getIteratorForTime is not available, we assume getSegmentReferenceNeeded_ updated internal state
                  if (iter.current() === refToFetch) {
                      iter.next();
                  }
                  
                  mediaState.lastSegmentReference = refToFetch;
                  if (!mediaState.waitingToClearBuffer) {
                      this.playerInterface_.onSegmentAppended(refToFetch, currentStream);
                  }
                  
                  mediaState.mitigation_warmupCount_++;
              }
          } catch (e) { }
      }
      
      // === BRANCH B: BURST PATH (Multiplexing) ===
      else {
          const fetchOperations = [];
          const fetchTypes = [];
          const realRefs = [];

          // 1. Prepare Decoy Request (Native Fetch)
          if (this.mitigation_shouldFetchDecoy_) {
               try {
                   // A. Calculate Size (0.2 to 0.7 of estimated segment)
                   let baseSize = 0;
                   if (reference.endByte != null) {
                       // Exact size known (DASH)
                       baseSize = reference.endByte - reference.startByte + 1;
                   } else {
                       // Estimate size (HLS): Bandwidth (bits/s) * Duration (s) / 8 = Bytes
                       // Default to 500KB if bandwidth is missing
                       const bandwidth = mediaState.stream.bandwidth || 4000000; 
                       const duration = reference.endTime - reference.startTime;
                       baseSize = (bandwidth * duration) / 8;
                   }

                   const randomFactor = 0.2 + (Math.random() * 0.5); // 0.2 to 0.7
                   const decoySize = Math.floor(baseSize * randomFactor);
                   
                   const decoyStart = reference.startByte || 0;
                   const decoyEnd = decoyStart + decoySize;

                   // B. Generate URI with Nonce
                   const uris = reference.getUris();
                   const originalUri = uris[0];
                   const separator = originalUri.includes('?') ? '&' : '?';
                   const nonceUri = originalUri + separator + 'd=' + Math.floor(Math.random() * 1e9);

                   // C. Native Fetch (Bypasses Shaka's strict checks)
                   const decoyHeaders = {
                       'Range': 'bytes=' + decoyStart + '-' + decoyEnd
                   };
                   
                   // Fire Native Request
                   // We catch errors immediately so they don't break Promise.all
                   const decoyPromise = fetch(nonceUri, { headers: decoyHeaders })
                       .then(response => {
                           // Optional: Read body to ensure network activity completes, then discard
                           return response.arrayBuffer(); 
                       })
                       .catch(e => {
                           // Ignore network errors for decoy
                       });

                   fetchOperations.push(decoyPromise);
                   fetchTypes.push('decoy');
          ...
          // 2. Prepare Real Segments (Shaka Fetch)
          // We loop through the number of segments we decided to fetch
          for (let i = 0; i < this.mitigation_segmentsToFetch_; i++) {
              const refToFetch = iter.current();
              if (!refToFetch) break;

              // Use internal fetch_() for real data to keep Shaka features (Retry, Auth, etc)
              fetchOperations.push(this.fetch_(mediaState, refToFetch));
              fetchTypes.push('real');
              realRefs.push(refToFetch);

              iter.next();
          }

          // 3. Fire Parallel Requests (Multiplexing)
          mediaState.performingUpdate = false;
          let results = [];
          try {
              if (fetchOperations.length > 0) {
                  // Abort if buffer clearing happened while prepping
                  if (mediaState.waitingToClearBuffer) {
                      throw new Error('Abort: Clearing Buffer');
                  }
                  
                  shaka.log.v1(logPrefix, 'Mitigation: Multiplexing ' + fetchOperations.length + ' requests.');
                  results = await Promise.all(fetchOperations);
              }
          } catch (e) {} 
          mediaState.performingUpdate = true;
          ...
          if (!mediaState.waitingToClearBuffer) {
              ...
              for (let i = 0; i < results.length; i++) {
                  if (mediaState.waitingToClearBuffer) break;

                  const type = fetchTypes[i];
                  const result = results[i];

                  if (type === 'real') {
                      const ref = realRefs[realIndex];
                      realIndex++;
                      ...
                      if (ref && result) {
                          try {
                              ...
                              // Use Locked Stream (currentStream)
                              await this.append_(
                                  mediaState, 
                                  presentationTime, 
                                  currentStream, // <--- LOCKED STREAM
                                  ref, 
                                  result
                              );
                              ...
                              mediaState.lastSegmentReference = ref;
                              if (!mediaState.waitingToClearBuffer) {
                                  this.playerInterface_.onSegmentAppended(ref, currentStream);
                              }
                              
                              mediaState.mitigation_warmupCount_++;

                          } catch (e) {
                              shaka.log.error(logPrefix, 'Mitigation: Append failed', e);
                              break;
                          }
                      }
                  }
                  // Decoy results are ignored automatically
              }
          }
      }
    ...
    /// === END OF SOLUTION BLOC 6 ===

    return null;
  }

  // Helper with strict seek-check
  cleanupAndReschedule_(mediaState, maxSegmentDuration) {
      if (mediaState.type == 2 /*VIDEO*/) this.mitigation_videoBusy_ = false;
      else if (mediaState.type == 1 /*AUDIO*/) this.mitigation_audioBusy_ = false;
      
      mediaState.mitigation_inBurstLoop_ = false;
      mediaState.performingUpdate = false;

      // === CRITICAL FIX: STOP IF CLEARING BUFFER ===
      // If Shaka wants to clear buffer, we must NOT schedule an update.
      // The seek/adaptation logic will restart the loop when ready.
      if (mediaState.waitingToClearBuffer) {
          return;
      }
      ...
      let nextDelay = maxSegmentDuration;
      try {
          const now = this.playerInterface_.getPresentationTime();
          const currentBuffer = this.playerInterface_.mediaSourceEngine.bufferedAheadOf(mediaState.type, now);
          if (currentBuffer < maxSegmentDuration * 1.5) nextDelay = 0;
      } catch(e) {}

      this.scheduleUpdate_(mediaState, nextDelay); 
  }
  /// TO DO : HANDLE THE 2ND SEGMENT OF BIG BUCK BUNNY + Find correct probas and prove them
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference, adaptation) {
    ...
    try {
      ...
      /*// === Solution: Making the FetchAndAppend skip its logic of segment fetching ===
 ...
 * @private
 */
shaka.media.StreamingEngine.MAX_RUN_AHEAD_SEGMENTS_ = 2; /// === SOLUTION BLOC 1 === Changed the max number from 1 --> 2 ===  END OF SOLUTION BLOC 1 ===  ///
